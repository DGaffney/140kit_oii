class ImporterTask
  ...
  def import_file_tsv
    ...
    DataMapper.repository.adapter.execute("load data infile '#{file_location}' ignore into table #{model.storage_name} fields terminated by '\\t' optionally enclosed by '\"' lines terminated by '\\0' ignore 1 lines (#{header.join(", ")});\n")
